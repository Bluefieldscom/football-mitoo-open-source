...
<cfelse> <!--- Long term pagegrabber introduced 14th May 2013 --->
...
<cfif SuppressLeagueTable IS "Yes">
	<cfif ListFind("Silver,Skyblue",request.SecurityLevel) >
		<table width="100%" border="0" cellspacing="2" cellpadding="0" >
			<tr>
				<cfoutput>
				<td align="CENTER"><span class="pix10">This #QKnockOut.CompetitionDescription# League Table has been hidden from the public</span></td>
				</cfoutput>
			</tr>
		</table>
	<cfelse>
...
<cfif ListFind("Silver,Skyblue",request.SecurityLevel) >
	<!--- bypass subsite tags if logged in as JAB or Administrator --->
<cfelse>
	<cfif DefaultYouthLeague IS 0> <!--- Adult League --->
		...
		<script type="text/javascript">
		sas_pageid='23489/160641';	// Page : FootballMitoo/homepage_over18
	...
	<cfelseif DefaultYouthLeague IS 1> <!--- Youth League --->
		...
		<script type="text/javascript">
		sas_pageid='23489/161126';	// Page : FootballMitoo/homepage_under18
		sas_formatid=9855;		// Format : Popunder1x1 1x1
		sas_target='';			// Targeting
		SmartAdServer(sas_pageid,sas_formatid,sas_target);
		</script>
