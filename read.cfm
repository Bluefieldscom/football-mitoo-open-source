<cfoutput>
	...
	<cfset arrSheets = objPOI.ReadExcel( 
		FilePath = ExpandPath( "./HEN2008.xls" ),
		HasHeaderRow = true
		) />
	...
	<cfloop
		...
		<!--- 
			...
			We know the structure of the Excel, so we can
		...
		<!---<cfdump var="#objSheet.Query#"><cfabort>--->
		...
		<cfloop query="objSheet.Query">
			...
			<cfif Len( objSheet.Query.column1 )>
				...
				<tr>
					...
					<td>
						#dateformat(objSheet.Query.column5,'YYYY-MM-DD')#
					</td>
				</tr>
