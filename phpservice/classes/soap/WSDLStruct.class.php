 ...
 *
 * TODO: more documentation on the required documentation
 *
 * @author David Kingma
 * @version 0.5
 */
class WSDLStruct {
	...
	public function __construct($tns, $url, $type = SOAP_RPC, $use = SOAP_ENCODED){
		...
		$this->xmlSchema = new XMLSchema($this->xsdSchema);
	 ...
	 *
	 * @param phpdocClass The service
	 * @return void
	 */
	public function setService(phpdocClass $class){
		$this->services[$class->name] = $class;
		$this->services[$class->name]->getMethods(false, false);
	}
	/**
	...
	public function generateDocument(){
		...
		foreach((array)$this->services as $serviceName => $service){
			...
			foreach((array)$service->methods as $operation){
				...
				if(substr($operationName, 0, 2) == "__") continue;
	...
	private function addMessage($name, $parts){
		...
		foreach((array)$parts as $partName => $partType){
			...
			if($t=XMLSchema::checkSchemaType(strtolower($partType)))
