...
<cfif NOT StructKeyExists(form, "StateVector") >
	<cfinclude template = "queries/qry_GetRefAvailable.cfm">
	<cfform name="UpdDelRefAvailable" action="RefAvailableUpdDel.cfm">
		<cfoutput>
			...
			<input name="ID" type="hidden" value="#GetRefAvailable.ID#">
			...
			<table width="100%" cellpadding="2" cellspacing="2" bgcolor="FEFFD2">
				...
				<tr align="center">
					<td colspan="2">
						<textarea name="Notes" rows="1" cols="30" class="textarea" >#GetRefAvailable.Notes#</textarea><br /><span class="textwarning">Notes (255 characters maximum)</span>
					</td>
				</tr>
				<tr align="center">
...
<cfelse>
	<cfif form.Action IS "Yes"> <!--- referee is definitely available --->
		<cfinclude template = "queries/upd_RefAvailable.cfm">
	<cfelseif form.Action IS "No"> <!--- referee is definitely unavailable --->
		<cfinclude template = "queries/upd_RefAvailable.cfm">
	<cfelseif form.Action IS "Delete"> 
		<cfinclude template = "queries/del_RefAvailable.cfm">
	<cfelse>
		ERROR in RefAvailableUpdDel.cfm
		<cfabort>
	</cfif>
	<!--- reload the calendar --->
	<cfoutput>
