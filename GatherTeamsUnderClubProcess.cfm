...
<cfif NOT StructKeyExists(form, "StateVector")>
	...
	<form ACTION="GatherTeamsUnderClubProcess.cfm?LeagueCode=<cfoutput>#LeagueCode#</cfoutput>" METHOD="POST">
		<input type="Hidden" name="StateVector" value="1">
		<table width="100%">
			<cfif ListFind("RogerPerry1,AdrianMarson2,JulianBauer3",request.SecurityLevel) >
			<cfelse>
				<tr>
					<td align="CENTER"><span class="pix10">Password </span><input name="ThisPerson" type="password" size="12"></td>		
				</tr>
			</cfif>
			<tr>
				<td align="CENTER">
					<table border="0" cellspacing="0" cellpadding="5" style="bg_pink">
						<tr>
							<td><select name="CountyInfo" size="1">
								<cfoutput query="QGetCountyInfo"><option value="#ID#^#CountyName#" <cfif StructKeyExists(url,"CIID") AND ID IS url.CIID>selected</cfif>>#CountyName#</option></cfoutput>
								</select>
							</td>
						</tr>
						<tr>
...
<cfelseif StructKeyExists(form, "StateVector") AND Form.StateVector IS 1>
	...
	<cfif StructKeyExists(form, "ThisPerson")>
		<cfset request.SecurityLevel = "#form.ThisPerson#" >
		<cfif ListFind("RogerPerry1,AdrianMarson2,JulianBauer3",request.SecurityLevel) >		
	...
	<cfelse>
		...
		<cfif NOT ListFind("RogerPerry1,AdrianMarson2,JulianBauer3",request.SecurityLevel) >		
			<cfset request.SecurityLevel = "White" >
			<cflock scope="session" timeout="10" type="exclusive">
				<cfset session.SecurityLevel = request.SecurityLevel >
				<cfif StructKeyExists(session, "LeagueCodePrefix") >
...
<cfelseif StructKeyExists(form, "StateVector") AND Form.StateVector IS 2 AND StructKeyExists(form, "ClubInfo") AND form.ClubInfo IS NOT "AddNewClub" >
	...
	**************************************
	--->
	<cfoutput>
	<form ACTION="GatherTeamsUnderClubProcess.cfm?LeagueCode=#LeagueCode#" METHOD="POST">
		<input type="Hidden" name="StateVector" value="2">
		...
		<cfset ClubInfoID = GetToken(form.ClubInfo, 1, '^' ) >
		<cfif StructKeyExists(form, "ClubName") AND Len(Trim(form.ClubName)) GT 0 >
			<cfset ClubName = form.ClubName >
			<cfinclude template="queries/upd_ClubInfo.cfm"> <!--- update the ClubInfo record with the modified club name --->
		<cfelse>
			<cfset ClubName = GetToken(form.ClubInfo, 2, '^' ) >
		</cfif>
		
		<cfif StructKeyExists(form, "Action") AND form.Action IS "Finished" >
			<CFLOCATION URL="GatherTeamsUnderClubProcess.cfm?LeagueCode=#LeagueCode#&CIID=#CountyInfoID#" ADDTOKEN="NO">
		</cfif>
			...
			<tr>
				...
				<td align="CENTER">
					<table border="0" cellspacing="0" cellpadding="2" >
						...
						<tr align="center">
							<td align="justify" bgcolor="white" ><span class="pix10navy"> you can now modify the <br> club name if necessary <td ></span><input name="ClubName" type="text" value="#ClubName#" size="30"></td>
						</tr>
					</table>
				</td>
				</cfoutput>
