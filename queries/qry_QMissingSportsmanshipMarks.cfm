<!--- Called by MissingSportsmanshipMarks.cfm --->
<cfquery name="QMissingSportsmanshipMarks" datasource="#request.DSN#" >
	SELECT
	...
	WHERE
		...
		AND f.ID NOT IN 
			(SELECT ID 
				...
				WHERE LeagueCode = <cfqueryparam value = '#request.filter#' cfsqltype="CF_SQL_VARCHAR" maxlength="5">
					AND (FixtureNotes LIKE '%TEAM SHEET MISSING%' OR PrivateNotes LIKE '%IgnoreMissingSportsmanshipMarks%'))  
		...
		AND (f.HomeSportsmanshipMarks IS NULL OR f.AwaySportsmanshipMarks IS NULL) 
		AND f.HomeID = c1.ID 
		AND f.AwayID = c2.ID 
		AND t1.ID = c1.TeamID 
	...
	ORDER BY
		FixtureDate DESC,  d.MediumCol, r.ShortCol
</cfquery>
