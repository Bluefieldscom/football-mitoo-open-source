...
<cfquery Name="DeregisteredPlayersByDate" datasource="#request.DSN#">
	SELECT
	...
	WHERE
		...
		AND r.LastDay IS NOT NULL
		AND	(r.LastDay BETWEEN '#request.Date001#' AND '#request.Date002#')
		AND r.TeamID = t.ID
		AND r.PlayerID = p.ID
	ORDER BY r.LastDay, ClubName, Surname, Forename
</cfquery>
