...
<!--- List of ALL the Fixtures & Results for the specified League and Match Day  
just for the highlighted club  --->
...
WHERE
	...
	AND (t1.ID = #request.fmTeamID# OR t2.ID = #request.fmTeamID#)
	AND FixtureDate = #CreateODBCDate(MDate)# 
ORDER BY
	SortOrder, <cfif KickOffTimeOrder>KOTime,</cfif> MatchNumber, HomeTeam, HomeOrdinal, AwayTeam, AwayOrdinal
</CFQUERY>
