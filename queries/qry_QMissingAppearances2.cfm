<!--- Called by MissingAppearances2.cfm --->

<cfquery name="QMissingAppearances2" datasource="#request.DSN#" >
	SELECT
		'Home' as MType,
	...
	WHERE
		...
		AND f.HomeTeamSheetOK = 0
		AND f.AwayTeamSheetOK = 1
		AND f.HomeID = c1.ID 
		AND f.AwayID = c2.ID 
		...
		AND d.id NOT IN 
			(SELECT ID 
				...
				WHERE 
					...
					AND (Notes LIKE '%EXTERNAL%' OR Notes LIKE '%HideDivision%'))		
	UNION
	SELECT
		'Away' as MType,
		t1.longCol as HomeTeam ,
	...
	WHERE
		...
		AND f.ID NOT IN (#ResultHAList#) 
		AND f.FixtureDate < #BoundaryDate# 
		AND f.HomeTeamSheetOK = 1
		AND f.AwayTeamSheetOK = 0
		AND f.HomeID = c1.ID 
		AND f.AwayID = c2.ID 
		AND f.RefereeID = r.ID 
		AND t1.ID = c1.TeamID 
